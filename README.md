<div align="center">

# 👋🏻 Hi, I'm Pierre Angelo Algarín Vigna

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=20&duration=2800&pause=800&color=00FF41&center=true&vCenter=true&width=800&lines=Ruby+on+Rails+Specialist+%F0%9F%9A%80;Backend+Developer+%7C+PostgreSQL+%26+MySQL+%E2%9A%99%EF%B8%8F;Angular+%7C+React+%7C+Vite+%7C+TypeScript+%F0%9F%92%BB;AWS+S3+%26+CloudFront+%7C+Cloud+Integrations+%F0%9F%93%A6;Always+Learning+and+Building+%F0%9F%93%9A)](https://github.com/pierreAngeloAA)

[![Profile Views](https://komarev.com/ghpvc/?username=pierreAngeloAA&color=00FF41&style=flat-square&label=Profile+Views)](https://github.com/pierreAngeloAA)
&nbsp;
[![GitHub followers](https://img.shields.io/github/followers/pierreAngeloAA?label=Followers&style=flat-square&color=00FF41&labelColor=0D1117)](https://github.com/pierreAngeloAA?tab=followers)

</div>

---

Backend Developer con experiencia construyendo y manteniendo aplicaciones web con **Ruby on Rails** (mi especialidad) y frameworks modernos de JavaScript. Enfocado en integraciones de API, sistemas de autenticación y mejoras de arquitectura backend, colaborando de la mano con frontends en Angular y React.

---

## ⚙️ Stack

- **Backend:** Ruby, Ruby on Rails
- **Frontend:** Angular, React, Vite, TypeScript, JavaScript, HTML, CSS
- **Estilos:** Tailwind CSS, Bootstrap, MaterializeCSS
- **Bases de datos:** PostgreSQL, MySQL
- **Cloud:** AWS (S3, CloudFront)
- **Herramientas:** Git, GitHub, Docker (básico), Jira, Slack, Postman
- **IA aplicada al desarrollo:** ChatGPT, Claude

---

## 🎯 Enfoque actual

- Arquitectura backend con Service Objects en Rails
- Integraciones de APIs externas y autenticación
- Migraciones de sistemas y mejoras de infraestructura backend
- Colaboración backend–frontend con Angular
- Buenas prácticas ágiles y código mantenible

---

## 👾 Contribution Graph

<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://github.com/pierreAngeloAA/pierreAngeloAA/raw/output/pacman-contribution-graph-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://github.com/pierreAngeloAA/pierreAngeloAA/raw/output/pacman-contribution-graph.svg" />
  <img alt="Pacman contribution graph" src="https://github.com/pierreAngeloAA/pierreAngeloAA/raw/output/pacman-contribution-graph.svg" />
</picture>

</div>
